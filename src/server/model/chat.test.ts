...
describe("Chat group", () => {
  ...
  test("Make a group", async () => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await pm.setPos(p.id, mm.room_id, { x: i, y: i }, "down")
      users.push(p.id)
    }
  ...
  test("Cannot make a group with a remote person", async () => {
    ...
    const pos1 = { x: 3, y: 3 }
    const users: UserId[] = []
    let p = await createUser(mm.room_id, pm)
    const pos2 = rand_non_adjacent(pos1, rows, cols)
    await pm.setPos(p.id, mm.room_id, pos2, "down")
    users.push(p.id)
    p = await createUser(mm.room_id, pm)
    await pm.setPos(p.id, mm.room_id, pos1, "down")
    users.push(p.id)
