...
/*
pub async fn get_room_2(
    data: web::Data<MyData>,
    _auth: Auth,
    path: web::Path<String>,
) -> HttpResponse {
    let start = Instant::now();
    // let conn = data.pgx.get().await.unwrap();
    let end = start.elapsed();
    // println!("connected {} µs", end.subsec_nanos() / 1000);
    let start = Instant::now();
    ...
    println!("query done {} µs", end.subsec_nanos() / 1000);
    let start = Instant::now();
    let ss = rows.iter().map(|r: &MapCellRDB| MapCell {
        id: r.id.clone(),
        room: r.room.clone(),
        x: r.x as usize,
        y: r.y as usize,
        kind: r.kind.clone(),
        poster_number: r.poster_number,
        custom_image: r.custom_image.clone(),
    });
    let cell_list: Vec<MapCell> = ss.collect();
