...
import { redis, log, dbWith, db as dbDefault, pgp, maps } from "./index"

export class PeopleModel {
  initialized = false
  db: pg.IDatabase<unknown, any>

  constructor(connection?: string) {
    this.db = connection ? dbWith(connection) : dbDefault
  }

  get_pos_pg = new pg.PreparedStatement({
    name: "get_pos",
  ...
  async isAdmin(token: string): Promise<boolean> {
    try {
      ...
      if (email) {
        const user_id = await this.getUserIdFromEmail(email)
        if (user_id) {
          const typ = await this.getUserType(user_id)
          return typ == "admin"
        }
      }
      return false
  ...
  async create(
    ...
    const uid = PeopleModel.genUserId()
    const count = (
      await this.db.query(`SELECT count(*) from person where name=$1;`, [name])
    )[0].count
    ...
    try {
      ...
      const uid_actual: string = (
        await this.db.query(`SELECT id FROM person WHERE email=$1`, [email])
      )[0].id
      ...
      if (merge_strategy == "replace") {
        await this.db.query(`DELETE FROM person_room_access WHERE person=$1;`, [
          uid_actual,
        ])
      }
      for (const room of allowed_rooms) {
        ...
        if (pos) {
          const _person = await this.db.query(
  ...
  async getAdmin(): Promise<UserId[] | null> {
  ...
  async getUnwrap(
    ...
    const p = await this.get(user_id, with_email, with_room_access)
    if (p) {
      return p
    } else {
  ...
  async getPos(
    ...
    if (use_redis) {
      ...
      this.get_pos_pg.values = [room_id, user_id]
      const row = await this.db.oneOrNone(this.get_pos_pg)
  ...
  async get_multi(
    ...
    const ps = await this.getAllPeopleList(null, with_email, with_room_access)
  ...
  // For moving, you need to call a function `MapModel.movePerson`, instead of this.
  async set(
    person_id: UserId,
    data: {
      name?: string
    ...
    await this.db.query(q + condition)
    if (data.email) {
  ...
  async set_stats(pid: UserId, stats: PersonStat): Promise<void> {
  ...
  async update(
    ...
    const person = await this.get(user_id)
    if (!person) {
      return null
    } else {
      log.debug(obj)
      ...
      await this.set(user_id, new_person)
  ...
  async getUserIdFromJWTHash(hash: string): Promise<UserId | null> {
    return await redis.accounts.get("hash:" + hash)
  }
  async getUserType(user_id: UserId): Promise<"admin" | "user" | null> {
  ...
  async verifySocketToken({
    ...
    if (debug_as) {
      if (debug_as == user && token == process.env.DEBUG_TOKEN) {
        ...
        const is_admin = (await this.getUserType(debug_as)) == "admin"
        return is_admin
      }
    }
    const sender_id = await this.getUserIdFromJWTHash(token)
    if (!sender_id) {
      return false
    } else if (sender_id && sender_id == user) {
      return true
    } else {
      const is_admin = (await this.getUserType(sender_id)) == "admin"
      return is_admin
    }
  }
  async isConnected(room_id: RoomId, user_ids: UserId[]): Promise<boolean[]> {
  ...
  async setDirection(user_id: UserId, direction: Direction): Promise<void> {
    if (!this.initialized) {
      throw new Error("Not initialized")
    }
    const person = await this.get(user_id)
  ...
  async getAll(room_id: RoomId | null): Promise<{ [index: string]: Person }> {
    if (!this.initialized) {
      throw new Error("Not initialized")
    }
    const people_list = await this.getAllPeopleList(room_id)
    return _.keyBy(people_list, "id")
  }

  static genUserId(): string {
    for (;;) {
